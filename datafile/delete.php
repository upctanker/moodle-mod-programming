<?php
    ...
    if ($confirmed) {
        delete_records('programming_datafile', 'id', $id);
        programming_datafile_adjust_sequence($a);
        add_to_log($course->id, 'programming', 'datafile_delete', "datafile/delet.php?a={$programming->id}", 'delete data file');
        redirect("list.php?a=$programming->id", get_string('datafiledeleted', 'programming'));
    }

/// Print the page header
    $pagename = get_string('deletedatafile', 'programming');
...
/// Print page content
    ...
    echo '<h2 class="main">'.get_string('deletedatafileconfirm', 'programming').'</h2>';
